...
@PostMapping
public ExchangeValue createEmployee(@RequestBody ExchangeValue exchangeValue) {
	return currencyExchangeService.createEmployee(exchangeValue);
}
...
@GetMapping("/currency-exchange-hello") //where {from} and {to} are path variable  
public String helloworld()  //from map to USD and to map to INR  
{     
    return "Hello world from my container"; 
}
}  